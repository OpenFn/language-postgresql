    ...
    try {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var structureData = data.map(function (x) {
          return "".concat(x.name, " ").concat(x.type, " ").concat(x.unique ? 'UNIQUE' : '', " ").concat(x.identity ? 'GENERATED BY DEFAULT AS IDENTITY' : '', " ").concat(x.required ? 'NOT NULL' : '', " ").concat(x["default"] ? "DEFAULT '".concat(x["default"], "'") : '');
        }).join(', ');
